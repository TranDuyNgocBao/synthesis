...
class Box:
    ...
    def draw(self, win, x, y, scl, last):
        scl = 1/scl
        ...
        if last:
            pygame.draw.polygon(win, (255, 0, 0), Polys, 1)
        else:
    ...
    def drawBolder(self, win, boxes):
        ...
        for box in boxes:
            if box.stt == 1:
                i = boxes.index(box)
                ii = i
                if len(boxes) > self.MAX:
                    ii = i + self.MAX - len(boxes) - 2
...
def Show(win, scr_size, element, boxes):
    ...
    for box in boxes:
        if box.stt == 0:
            ...
            if box == boxes[-1]:
                box.draw(win, x, y, scl, True)
            else:
                box.draw(win, x, y, scl, False)
